...
- README: new "Tested printers" table (printer, system, what was checked, result) and a link to the new bug report form on the website, also in `CONTRIBUTING.md` and the issue chooser.
...
- **Resin printers with a resin vat:** Under “Manage printers” you now choose “Filament” or “Resin” next to the name when adding a printer; the kind is fixed afterwards, existing printers are filament printers. A resin printer gets exactly one “Resin vat” for one bottle instead of the spool holder; it can't be renamed or deleted, and resin printers have no further units and no printer connection. In the “Resin” view of the filament storage it appears in the right-hand column: insert a bottle by dragging it or via the vat's menu (which only lists resin bottles), the vat shows the bottle, its color and the remaining ml, “Unload” returns it to its home location, and “− Use” is available for the inserted bottle too. “Is there enough filament?”, the cost estimate and the queue icon ignore resin printers.

### Changed

- Resin bottles now only fit into the resin vat of a resin printer, and filament spools never go into a resin vat. While dragging, a slot that doesn't fit is no longer highlighted, the slot menu only lists matching entries, and a loaded entry can only be switched to the other kind after unloading it. The right-hand column of the filament storage shows only filament or only resin printers, depending on the view. Older catalogs and backups are converted automatically when opened.
...
- README: neue Tabelle „Getestete Drucker“ (Drucker, System, was geprüft wurde, Ergebnis) und ein Link zum neuen Formular „Fehler melden“ auf der Webseite, auch in `CONTRIBUTING.md` und in der Issue-Auswahl.

    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-[480px] max-h-[80vh] flex flex-col bg-[var(--panel)] border border-[var(--line)] rounded shadow-[var(--shadow)]">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3">
          {!hasIssues && <div className="text-[length:var(--font-size-title)] text-[var(--ink-3)]">{t('cleanupNoIssues')}</div>}
          ...
          {issues.duplicateGroups.map((group, groupIndex) => (
            <div key={groupIndex} className="pb-4">
              ...
              {group.map((model, index) => (
                <label
                  key={model.id}
                  className={`flex items-center gap-2 py-1 text-[length:var(--font-size-title)] ${index === 0 ? 'opacity-60' : 'cursor-pointer'}`}
                >
                  <input
                    type="checkbox"

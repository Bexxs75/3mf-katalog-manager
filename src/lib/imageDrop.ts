...
export function isOverDropZone(position: Point, devicePixelRatio: number, zone: RectLike | null): boolean {
  return zone !== null && isPointInRect(physicalToCss(position, devicePixelRatio), zone);
}
...
export function evaluateImageDrop(
  ...
  if (!isOverDropZone(position, devicePixelRatio, zone)) return { kind: 'outside' };
